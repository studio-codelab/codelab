...
INSTALLATION CODELAB : cette skill est installee dans le depot, pas comme plugin. Le chemin ci-dessous est donc relatif a la RACINE DU PROJET (le repertoire de travail par defaut de Claude Code ici). La forme d'origine, `${CLAUDE_PLUGIN_ROOT}/...`, ne fonctionnerait pas : cette variable n'est posee que par le mecanisme de plugins, indisponible dans cet environnement.
